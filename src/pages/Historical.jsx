...
import {
  ...
  Legend, ResponsiveContainer, Brush, Area,
} from 'recharts';
import ReactDatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';
import {
  fetchHistoricalWeather, fetchHistoricalAirQuality, toDateStr,
} from '../utils/api.js';
...
export default function Historical({ location }) {
  ...
  // Archive API has ~7 day lag, block last 7 days in picker
  const archiveSafeDate = new Date(today);
  archiveSafeDate.setDate(archiveSafeDate.getDate() - 7);

  const [startDate, setStartDate] = useState(subYears(today, 1));
  const [endDate, setEndDate] = useState(archiveSafeDate);
  ...
  const loadData = useCallback(async () => {
    ...
    if (diff < 1) {
      setError('Please select a range of at least 2 days.');
      return;
    }
  ...
  const buildAqDailyData = () => {
    ...
    return Object.entries(byDate).map(([date, vals]) => ({
      ...
      pm10: vals.pm10.length
        ? vals.pm10.reduce((a, b) => a + b, 0) / vals.pm10.length
        : null,
      pm25: vals.pm25.length
        ? vals.pm25.reduce((a, b) => a + b, 0) / vals.pm25.length
        : null,
    }));
  };
    ...
    <div className="hist-page container">
      <div className="hist-header">
        <div>
          ...
          <p className="hist-subtitle">
            Analyze up to 2 years of weather trends · {location?.city}
          </p>
        </div>
      </div>
        ...
        <div className="picker-row">
          <div className="picker-group">
            <label className="ds-label">
              Select Date Range (max 2 years · data available up to 7 days ago)
            </label>
            <ReactDatePicker
              selected={startDate}
      ...
      {!loading && data && (
        <div className="hist-charts fade-in">
          ...
          {aqDailyData.length > 0 ? (
            ...
            <div
              className="card"
              style={{ textAlign: 'center', color: 'var(--text2)', padding: '32px' }}
            >
      ...
      {!loading && !data && !error && (
        <div className="hist-empty">
          ...
          <p>
            Select a date range and click <strong>Fetch Data</strong> to view historical trends.
          </p>
        </div>
      )}
    </div>
